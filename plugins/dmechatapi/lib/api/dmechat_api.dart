...
class DmechatApi {
  DmechatApi([ApiClient apiClient]) : apiClient = apiClient ?? defaultApiClient;
  ...
  Future<Response> listAvailableUsersWithHttpInfo() async {
    final path = r'/list-available-users';
  ...
  /// list users in the app
  Future<List<UserModel>> listAvailableUsers() async {
    final response = await listAvailableUsersWithHttpInfo();
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, _decodeBodyBytes(response));
    }
    ...
    if (response.body != null && response.statusCode != HttpStatus.noContent) {
      return (apiClient.deserialize(_decodeBodyBytes(response), 'List<UserModel>') as List)
        .cast<UserModel>()
        .toList(growable: false);
    }
    return Future<List<UserModel>>.value(null);
  }
